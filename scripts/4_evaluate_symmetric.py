#!/usr/bin/env python
"""Evaluate symmetric TMO model (trained without attention bias): Δτ heatmap, clustering, LCS, and annotation.

Uses the same TMOLatentModelAsymmetric architecture but with use_bias=False.
"""
...
from tmo.models import TMOLatentModelAsymmetric          # updated import
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="./tmo_results/tmo_symmetric_best.pt")
    ...
    # Flip sign if pseudotime was reversed
    ...
    # 4. Prepare heatmap data and labels based on annotation_mode
    if args.annotation_mode == "component":
        ...
        if args.standardize:
            row_mean = dt_cluster.mean(axis=1, keepdims=True)
            row_std = dt_cluster.std(axis=1, keepdims=True)
            dt_cluster = (dt_cluster - row_mean) / (row_std + 1e-8)
            print("Standardized Δτ profiles for clustering.")
        dist = pdist(dt_cluster, metric='correlation')
        row_linkage = linkage(dist, method='ward')
        cluster_ids = fcluster(row_linkage, args.dist_threshold, criterion='distance')
        ...
        if args.annotation_mode == "cluster_deltatau":
            ...
            dt_heatmap = delta_tau_smooth[row_order, :]
            row_labels = [comp_names[i] for i in row_order]
        else:  # cluster_aggregate
            cluster_median = []
            cluster_label_list = []
            for cl in np.unique(cluster_ids):
                comp_indices = np.where(cluster_ids == cl)[0]
    ...
    fig1 = plot_delta_tau_heatmap(
        ...
        window_centers,
        row_labels,
        title="Regulatory lag Δτ(τ) (symmetric model)",
        save_path=str(output_dir / "fig1_latent_delta_tau_symmetric.pdf"),
    )
    plt.close(fig1)
    print("Saved latent Δτ heatmap (Figure 1).")

    # 6. Load model and compute LCS
    print("Loading symmetric model...")
    model = TMOLatentModelAsymmetric(             # same architecture as asymmetric
    ...
    if Path(args.model_path).exists():
        ...
        with torch.no_grad():
            ...
            out = model(rna_t, atac_t, pt_t, use_bias=False)   # symmetric pass
    ...
    # 7. Print top genes for first few components (if annotation available)
    if annot_file.exists() and args.annotation_mode != "cluster_aggregate":
        print("\nTop genes for the first 5 RNA components:")
        for i in range(min(5, n_atac)):
            comp = f"Comp{i}"
            row = annot_df[annot_df['Component'] == comp]
            if not row.empty:
                top_genes = row['Top_genes'].values[0]
                print(f"{comp}: {top_genes[:100]} ...")
    elif annot_file.exists() and args.annotation_mode == "cluster_aggregate":
        print("\nClusters aggregated, not printing per‑component top genes.")
    else:
        print("No annotation file; skipping top genes.")
